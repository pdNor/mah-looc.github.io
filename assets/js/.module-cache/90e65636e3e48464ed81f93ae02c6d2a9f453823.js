    ...
    var Domain = React.createClass({displayName: "Domain",
        ...
        handleDomainSelection: function() {
            ...
            if (allChecked) {
                ...
                var x = modules.map(function(module) {
                    React.findDOMNode(module).checked = false;
                    return module.props.module.id;
                });

                this.props.removeChoice(x);
            } else {
                console.log("iterating refs");
                var ms = [];
                for (var ref in this.refs) {
                    var r = this.refs[ref];
                    console.log("ref", r);
                    var node = React.findDOMNode(r);

                    if (!node.checked) {
                        console.log("is not checked");
                        node.checked = true;
                        ms.push(r.props.module);
                    }
                }

                this.props.addChoice(ms);
                // modules.forEach(function(module) {
