...
(function(window, document) {
    ...
    var PathForm = React.createClass({displayName: "PathForm",
        ...
        removeChoice: function(choice) {
            var choices = this.state.choices.filter(function(o) { return o.id != choice.id; });
            this.setState({ choices: choices });
        },
        handleClick: function() {
            console.log(this.state.choices);
            // .sort(function(x, y) { return +x.substr(1) - +y.substr(1); });
        },
        getInitialState: function() {
            return {
                domains: [],
        ...
        render: function() {
            return (
                React.createElement("div", {id: "path-form"}, 
                    ...
                    React.createElement("button", {type: "button", onClick: this.handleClick}, "Submit")
                )
            );
        }
    });
